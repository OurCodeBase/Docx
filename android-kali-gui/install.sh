...
sudo apt update && sudo apt install tigervnc-standalone-server udisks2 xfce4 tilda dbus-x11 -y && sudo rm /var/lib/dpkg/info/udisks2.postinst && sudo echo "" >> /var/lib/dpkg/info/udisks2.postinst && sudo dpkg --configure -a && sudo apt-mark hold udisks2 && mv $HOME/android-kali-gui/vncst* /usr/bin && vncstart && sudo apt-autoremove && sudo clear && echo -e "$Success Ready To Connect"